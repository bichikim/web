 ...
 * Build: `lodash modularize exports="es" -d -s -o src/lodash`
...
const mixin = (function (func) {
  return function (object, source, options) {
    if (options == null) {
      const isObj = isObject(source)
      const props = isObj && keys(source)
      const methodNames = props && props.length && baseFunctions(source, props)
...
baseForOwn(LazyWrapper.prototype, function (func, methodName) {
  ...
  const retUnwrapped = isTaker || methodName.startsWith('find')
  ...
  lodash.prototype[methodName] = function () {
    ...
    const isHybrid = this.__actions__.length > 0
    ...
    if (!retUnwrapped && useLazy) {
      ...
      result.__actions__.push({args: [interceptor], func: thru, thisArg: undefined})
...
baseForOwn(LazyWrapper.prototype, function (func, methodName) {
  ...
  if (lodashFunc) {
    ...
    names.push({func: lodashFunc, name: methodName})
  }
})
