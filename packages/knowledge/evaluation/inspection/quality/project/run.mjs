...
const [destination, version = '26', previous, searchMode = 'live', selected] = process.argv.slice(2)
assert.equal(searchMode, 'live')
...
const repository = baseline?.repository ?? (await mkdtemp(join(tmpdir(), 'knowledge-project-')))
assert.ok(basename(repository).startsWith('knowledge-project-'))
if (baseline === undefined) {
...
assert.equal(source.repoId, 'demo/inspection-project')
