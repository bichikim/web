...
assert.ok(['original', 'supplemental', 'missing'].includes(mode))
const version = '27'
...
const fingerprint = async () => ({
  ...
  corpus: await tree(join(here, 'corpus', mode)),
...
const repository = await mkdtemp(join(tmpdir(), 'knowledge-placement-'))
await cp(join(here, 'corpus', mode), repository, {recursive: true})
...
assert.equal(source.repoId, `demo/inspection-placement/${mode}`)
...
await save('input.json', {
  authority: definitions.authority,
  cases,
  frozen,
  indexed,
  model,
...
for (let repeat = 1; repeat <= REPEATS; repeat += 1) {
  for (const item of cases) {
    ...
    const relevant = new Set(
      source.points
        .filter((p) => item.requiredEvidence.includes(p.payload.docId))
        .map((p) => p.pointId),
    )
    const record = {
      actual: result.value?.kind,
      agrees: result.ok && item.expected.includes(result.value.kind),
      calls: trace.length,
      ...
      group: item.group,
      id: item.id,
      networkSearches: queries.length,
