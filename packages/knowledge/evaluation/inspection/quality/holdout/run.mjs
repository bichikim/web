...
const repository = baseline?.repository ?? (await mkdtemp(join(tmpdir(), 'knowledge-holdout-')))
assert.ok(basename(repository).startsWith('knowledge-holdout-'))
if (baseline === undefined) {
...
assert.equal(source.repoId, 'demo/inspection-holdout')
