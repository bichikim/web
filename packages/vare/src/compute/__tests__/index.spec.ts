...
describe('compute', () => {
  ...
  it('should have a name in the tree', () => {
    ...
    const nameDecoName = useInfo().get(tree.nameDeco)?.name
    const nameStaticDecoName = useInfo().get(tree.nameStaticDeco)?.name
    const nameReactiveDecoName = useInfo().get(relateTree.nameReactiveDeco)?.name

    expect(nameDecoName).toBe('nameDeco')
    expect(nameStaticDecoName).toBe('nameStaticDeco')
    expect(nameReactiveDecoName).toBe('nameReactiveDeco')
  })
  ...
  it('should be compute', () => {
    const {nameDecoSet} = setup()
    expect(isCompute(nameDecoSet)).toBe(true)
  })
})
