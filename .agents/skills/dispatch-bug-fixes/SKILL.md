...
4. Give each task its issue URL and require it to:
   ...
   - After the review gate and final fixes, run the full repository unit-test suite with `pnpm test`; targeted or package-scoped tests do not satisfy this requirement. Report failures or inability to run it as verification gaps, and never claim it passed unless it did.
   - For issues directly involving Neon `local-1` database behavior, use the provided local environment configuration (for Pomo tasks, `apps/pomo/.env.local`) to reproduce and verify the affected behavior directly. Do not expose secret values in task output, logs, or PRs. If the configuration is unavailable, the database is unreachable, or suitable test data is unavailable, report a verification gap.
   - After the gate passes, push the branch as needed and create a verified **draft** PR against `dev` with `Fixes #<issue-number>`. Require its body to include all P0–P2 findings across review passes with cause, change, and before/after verification; all confirmed P3/P4 findings with unique review numbers and disposition; and all verification gaps or inapplicable checks with reasons. Write `none` for empty categories. Finish with the draft PR URL and branch name. Retry recoverable PR failures; report exact evidence for an unrecoverable blocker instead of claiming completion.
