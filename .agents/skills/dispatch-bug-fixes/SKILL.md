...
4. In each task, inspect relevant callers, callees, types, tests, and configuration; verify the root cause; implement the smallest compatible fix; add regression coverage; and report changed files, evidence, verification, and limitations.
   Completion gate: invoke `$critical-review-fix-loop` immediately after the fix. It must perform the full P0–P4 review, fix every authorized issue-scope P0–P2 finding, and rerun the review and applicable unit tests, `typecheck`, lint, and formatting until no P0–P2 remains. Lint or build cannot replace tests or `typecheck`. If blocked, do not bypass the block or create the PR; report the blocker and evidence.
