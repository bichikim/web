...
1. If tests already exist, verify relevant current results in this strict order: Wallaby MCP, Wallaby CLI when MCP is unavailable, then Vitest only when neither Wallaby option is available or sufficient. Resolve failures relevant to the changed path within the authorized scope; report unrelated pre-existing failures with evidence without making their repair a prerequisite for adding tests.
...
9. Cover important behavior and failure paths. Report unverified paths; exclude coverage only with an independent justification, such as proven unreachability, and document it. Do not add ignore comments merely to reach a coverage target.
