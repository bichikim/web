...
Do not require a prop merely to simplify implementation, mirror one current caller, pair a value with an event callback by convention, or catch an event the caller may not need. When only one variant needs a value, require it in that variant's discriminated-union member instead of requiring it globally.
...
- Treat `onChange` and `onInput` as independent event callbacks, not mandatory companions to `value` or `defaultValue`.
- Keep event callbacks optional unless the component's sole purpose is impossible without that callback.
- Do not infer a controlled/uncontrolled mode solely from `value` or `defaultValue`; define only the behavior the component needs.
- For one-shot local-state initialization, prefer `initial*` props with `untrack`. Preserve `defaultValue` when it is a native platform or existing API term.
