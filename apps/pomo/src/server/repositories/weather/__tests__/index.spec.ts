...
it('should return the latest weather row for a location', async () => {
  const record = {
    collectedAt: new Date('2026-08-22T05:50:00.000Z'),
    humidityPercent: 50,
    id: 'weather-id',
  ...
  await expect(getLatestWeather('seoul', database)).resolves.toEqual(record)
})

it('should return undefined when no weather row has been collected', async () => {
  ...
  await expect(getLatestWeather('seoul', database)).resolves.toBeUndefined()
})
