...
describe('AI service', () => {
  it('redacts the private object key from status and signs a fresh result URL on access', async () => {
    ...
    repositoryMocks.findAiJobArtifactForUser.mockResolvedValue({
    ...
    expect(createPublicJob(succeeded).result).toEqual({
      artifact: {contentType: 'audio/wav'},
    })
    await expect(createAiJobResult(succeeded)).resolves.toEqual({
      artifact: {
        contentType: 'audio/wav',
    ...
    expect(artifactMocks.createAiArtifactDownloadUrl).toHaveBeenCalledWith(
      `ai/jobs/${JOB_ID}/archive/result.wav`,
      {expiresAt: null, now: NOW},
    )
    expect(repositoryMocks.findAiJobArtifactForUser).toHaveBeenCalledWith(JOB_ID, USER_ID)
  })
})
