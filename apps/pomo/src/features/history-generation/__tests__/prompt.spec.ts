...
it('should request a cohesive radio script without spoken section labels', () => {
  ...
  expect(HISTORY_PROMPT_VERSION).toBe('today-in-history-v4-required-title-order')
...
it('should constrain a regeneration to the requested stable titles in list order', () => {
  const prompt = buildHistoryPrompt({
    policy: {
      allowedDomains: ['example.com'],
      seedUrls: ['https://example.com/history'],
