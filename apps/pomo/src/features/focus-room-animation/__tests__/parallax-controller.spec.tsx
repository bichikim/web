...
const createController = (...args: ConstructorParameters<typeof ParallaxController>) => {
  const controller = new ParallaxController(...args)
  controllers.set(args[0], controller)
  return controller
}
...
const createPointerHost = () => {
  const view = render(() => (
    <div
      onPointerDown={(event) => controllers.get(event.currentTarget)?.onPointerDown(event)}
      onPointerMove={(event) => controllers.get(event.currentTarget)?.onPointerMove(event)}
      onPointerUp={(event) => controllers.get(event.currentTarget)?.onPointerUp(event)}
      onPointerCancel={(event) => controllers.get(event.currentTarget)?.onPointerCancel(event)}
    />
  ))
...
const runDragAnimation = (frameDuration: number) => {
  ...
  const controller = createController(host, renderOffset)
...
describe('ParallaxController', () => {
  ...
  it('should propagate runtime reduced-motion preference changes', () => {
    ...
    const controller = createController(document.createElement('div'), renderOffset, {
  ...
  it('should ignore pointer movement until the background is pressed', () => {
    const animationFrames = createAnimationFrames()
    const host = createPointerHost()
    const controller = createController(host, vi.fn())
  ...
  it('should ignore late permission results after mode changes or destruction', async () => {
    ...
    vi.stubGlobal('DeviceOrientationEvent', TestDeviceOrientationEvent)
    const first = createController(document.createElement('div'), vi.fn(), {
      inputMode: 'gyroscope',
    })
    first.start()
    window.dispatchEvent(createPointerEvent('pointerdown'))
    ...
    const second = createController(document.createElement('div'), vi.fn(), {
      inputMode: 'gyroscope',
    })
    second.start()
    window.dispatchEvent(createPointerEvent('pointerdown'))
    ...
    const third = createController(document.createElement('div'), vi.fn(), {
      inputMode: 'gyroscope',
    })
    third.start()
    window.dispatchEvent(createPointerEvent('pointerdown'))
