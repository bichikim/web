import type {Accessor} from 'solid-js'

import type {PActivity, PGaze} from '../focus-room-animation'
import type {SceneTimeMode} from '../focus-room-time'

export type {PActivity, PGaze} from '../focus-room-animation'

export const FOCUS_ROOM_TIME_OPTIONS = [
  {icon: 'i-tabler-sun', label: '낮', value: 'day'},
  {icon: 'i-tabler-moon', label: '밤', value: 'night'},
  {icon: 'i-tabler-sun-moon', label: '자동', value: 'auto'},
] as const satisfies readonly SceneOption<SceneTimeMode>[]

export const FOCUS_ROOM_ACTIVITY_OPTIONS = [
  {icon: 'i-tabler-book-2', label: '책 읽기', value: 'reading'},
  {icon: 'i-tabler-pencil', label: '글쓰기', value: 'writing'},
  {icon: 'i-tabler-keyboard', label: '노트북 타이핑', value: 'typing'},
] as const satisfies readonly SceneOption<PActivity>[]

export const FOCUS_ROOM_GAZE_OPTIONS = [
  {icon: 'i-tabler-focus-2', label: '작업에 집중', value: 'focused'},
  {icon: 'i-tabler-user-scan', label: '사용자 보기', value: 'user'},
] as const satisfies readonly SceneOption<PGaze>[]

interface SceneOption<TValue extends string> {
  readonly icon: string
  readonly label: string
  readonly value: TValue
}

export interface PScenePreferences {
  readonly activity: PActivity
  readonly gaze: PGaze
  readonly timeMode: SceneTimeMode
}

export interface PScenePreferencesController {
  readonly activity: Accessor<PActivity>
  readonly gaze: Accessor<PGaze>
  readonly isReady: Accessor<boolean>
  readonly onActivityChange: (activity: PActivity) => void
  readonly onGazeChange: (gaze: PGaze) => void
  readonly onTimeModeChange: (timeMode: SceneTimeMode) => void
  readonly timeMode: Accessor<SceneTimeMode>
}

export const DEFAULT_P_SCENE_PREFERENCES = {
  activity: 'reading',
  gaze: 'focused',
  timeMode: 'day',
} as const satisfies PScenePreferences
