...
interface DialogueTestRoot {
  readonly controller: QwenDialogueWriterController
  readonly dispose: () => void
}

interface TestRuntime extends QwenDialogueRuntime {
  readonly client: QwenClient
  readonly emit: (response: QwenWorkerResponse) => void
}

const createRuntime = (supported: boolean): TestRuntime => {
  let onResponse: ((response: QwenWorkerResponse) => void) | null = null
  ...
  return {
    ...
    emit: (response) => {
      if (onResponse === null) {
        throw new Error('Qwen client가 생성되지 않았습니다.')
      }
...
const createDialogueRoot = (runtime: QwenDialogueRuntime): DialogueTestRoot => {
  let disposeRoot: () => void = () => undefined
  const controller = createRoot((dispose) => {
    disposeRoot = dispose
    return useQwenDialogueWriter({
...
describe('useQwenDialogueWriter', () => {
