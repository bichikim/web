...
import DevPageDispatcher from '../DevPageDispatcher'
...
vi.mock('src/components/dev/HomePage', () => ({default: createPage('home')}))
vi.mock('src/components/dev/CharacterPage', () => ({default: createPage('character')}))
vi.mock('src/components/dev/ChatPage', () => ({default: createPage('chat')}))
vi.mock('src/components/dev/DialoguePage', () => ({default: createPage('dialogue')}))
vi.mock('src/components/dev/LayerReviewPage', () => ({default: createPage('layer-review')}))
vi.mock('src/components/dev/SpeechToTextPage', () => ({default: createPage('speech-to-text')}))
vi.mock('src/components/dev/TermsPage', () => ({default: createPage('terms')}))
vi.mock('src/components/dev/TextMoodPage', () => ({default: createPage('text-mood')}))
vi.mock('src/components/dev/VoicePage', () => ({default: createPage('voice')}))
...
it.each([
  ...
  render(() => <DevPageDispatcher fallback={<output>404</output>} pathname={pathname} />)
...
it('should preserve a development page URL with a trailing slash', async () => {
  render(() => <DevPageDispatcher fallback={<output>404</output>} pathname="/dev/chat/" />)
...
it('should own the development terms metadata', async () => {
  render(() => <DevPageDispatcher fallback={<output>404</output>} pathname="/dev/terms" />)
