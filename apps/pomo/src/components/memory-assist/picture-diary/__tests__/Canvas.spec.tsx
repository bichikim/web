...
it.each([true, false])(
  'should cancel a stroke after history changes with capture %s',
  (captured) => {
    const initialStroke = {
      points: [
        {x: 0.1, y: 0.1},
    ...
    Object.defineProperty(canvas, 'hasPointerCapture', {
      value: (pointerId: number) => captured && pointerId === 1,
    })
    Object.defineProperty(canvas, 'getBoundingClientRect', {
    ...
    expect(releaseCapture.mock.calls).toEqual(captured ? [[1]] : [])
