...
import {P_BUTTON_CLASSES, PButton} from './PButton'
...
export const TransparentTones: Story = {
  args: {transparent: true},
  render: ToneComparison.render,
}

export const TransparencyComparison: Story = {
  name: 'Transparency — Opaque and Translucent',
  play: async ({canvasElement}) => {
    const canvas = within(canvasElement)
  ...
  play: async ({canvasElement}) => {
    ...
    await Promise.all(
      ['Medium', 'Small'].map(async (size) => {
        ...
        const iconSize = size === 'Small' ? '18px' : '24px'
        const imageSize = size === 'Small' ? '24px' : '32px'
        await Promise.all(
          Array.from(icon.querySelectorAll('[aria-hidden="true"]'), async (element) => {
            await expect(getComputedStyle(element).width).toBe(iconSize)
            await expect(getComputedStyle(element).height).toBe(iconSize)
          }),
        )
        const leadingImage = image.querySelector('img')!
        await expect(getComputedStyle(leadingImage).width).toBe(imageSize)
        await expect(getComputedStyle(leadingImage).height).toBe(imageSize)
...
export const GlassIconSmall: Story = {
  ...
  name: 'Glass Icon — Small Custom Icon',
  play: async ({canvasElement}) => {
    const button = within(canvasElement).getByRole('button', {name: '설정 열기'})
    const icon = button.querySelector('[data-pomo-button-icon]')!
    await expect(button.getBoundingClientRect().width).toBe(32)
...
export const ClippedIconTooltip: Story = {
  ...
  name: 'Tooltip — Clipped Container',
  play: async (context) => {
...
export const StyledLink: Story = {
  render: (args) => (
    <a href="#button-link" class={P_BUTTON_CLASSES({...args, class: 'no-underline'})}>
      돌아가기
    </a>
  ),
}
