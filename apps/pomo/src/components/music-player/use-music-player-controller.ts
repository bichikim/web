...
// oxlint-disable-next-line eslint/max-lines-per-function, eslint/max-statements -- Playback lifecycle shares one media element and revision state.
export const useMusicPlayerController = (props: PMusicPlayerContentProps) => {
  ...
  const previewPlayback = createPreviewPlayback({
    isPlaying,
    pausePlayer: () => audioElement?.pause(),
