...
describe('TrainCanvas setup', () => {
  ...
  it('should disable auto-rotation when its behavior is undefined', async () => {
    ...
    render(() => <TrainCanvas autoRotate={false} modelUrl="/character.glb" {...callbacks} />)
    await waitFor(() => expect(callbacks.onLoadSuccess).toHaveBeenCalledOnce())
...
describe('TrainCanvas loading', () => {
  it('should retain the cabin and its camera while switching characters', async () => {
    ...
    const view = render(() => (
      <TrainCanvas trainCabin cacheModels modelUrl={modelUrl()} {...callbacks} />
    ))
  ...
  it('should reuse cached models without loading again and dispose them on unmount', async () => {
    ...
    const callbacks = createCallbacks()
    const [modelUrl, setModelUrl] = createSignal('/first.glb')
    const view = render(() => <TrainCanvas cacheModels modelUrl={modelUrl()} {...callbacks} />)
    await waitFor(() => expect(callbacks.onLoadSuccess).toHaveBeenCalledTimes(1))
    setModelUrl('/second.glb')
    await waitFor(() => expect(callbacks.onLoadSuccess).toHaveBeenCalledTimes(2))
  ...
  it('should dispose a stale success and ignore its progress', async () => {
    ...
    const callbacks = createCallbacks()
    const [modelUrl, setModelUrl] = createSignal('/first.glb')
    render(() => <TrainCanvas modelUrl={modelUrl()} {...callbacks} />)
...
describe('TrainCanvas camera fitting', () => {
  ...
  it('should skip fitting non-finite bounds', async () => {
    const callbacks = createCallbacks()
    render(() => <TrainCanvas modelUrl="/invalid.glb" {...callbacks} />)
