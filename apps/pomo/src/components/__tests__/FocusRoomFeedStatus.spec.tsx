...
it('should show a ready feed notice', () => {
  vi.mocked(useFocusRoomFeedContext).mockReturnValue(createFeeds())

  render(() => <FocusRoomFeedStatus />)
...
it('should play all accumulated feed dialogues with one action', () => {
  ...
  vi.mocked(useFocusRoomFeedContext).mockReturnValue(feeds)
...
it('should hide the ready notice while queued dialogues are playing', () => {
  vi.mocked(useFocusRoomFeedContext).mockReturnValue(createFeeds([READY_DIALOGUE], true))
