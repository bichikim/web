...
it.each([undefined, {invalidResult: 'Custom invalid result.', readFailed: 'Custom read failure.'}])(
  'should preserve or wrap the original reader error with messages %j',
  async (messages) => {
    const reader = new FileReader()
...
it.each([undefined, {invalidResult: 'Custom invalid result.', readFailed: 'Custom read failure.'}])(
  'should reject a non-buffer result with messages %j',
  async (messages) => {
    const reader = new FileReader()
    vi.spyOn(reader, 'readAsArrayBuffer').mockImplementation(() => {
