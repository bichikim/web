...
describe('handleUserAuthRequest', () => {
  ...
  it('should protect account responses from caching and referrer leaks', async () => {
    const headers = new Headers()

    await expect(
      handleUserAuthRequest({
        request: new Request('https://pomo.example/account'),
        responseHeaders: headers,
        url: new URL('https://pomo.example/account'),
      }),
    ).resolves.toBeNull()
