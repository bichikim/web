...
describe('bug-hunt: delayed-end speech interruption', () => {
  beforeEach(() => {
    vi.useFakeTimers()
  ...
  it('should stop active one-off speech before delayed-end dialogue playback', async () => {
    const stopExternalSpeech = vi.fn()
    const view = renderHook(() => usePEventController({}))
    await vi.waitFor(() => expect(view.result.isLoading()).toBe(false))

    await view.result.playDialogueEvents(['delayed-end'], stopExternalSpeech)
    expect(stopExternalSpeech).toHaveBeenCalledOnce()
    stopExternalSpeech.mockClear()
    playback.playSequence.mockClear()
