...
test('should use quick controls and persist a completed-session reset', async ({
  page,
}, information) => {
  await openHome(page)
  const dialog = await openTimer(page)
  ...
  const controls = page.getByRole('group', {exact: true, name: '포모도로 간편 조작'})
  const trigger = controls.getByRole('button', {name: /^포모도로 열기,/u})
  await controls.getByRole('button', {exact: true, name: '집중 시작'}).click()
  await page.clock.fastForward(10_000)
  await controls.getByRole('button', {exact: true, name: '일시정지'}).click()
  await expect(trigger).toHaveAccessibleName('포모도로 열기, 일시정지, 00:50')
  await page.clock.fastForward(60_000)
  await expect(trigger).toHaveAccessibleName('포모도로 열기, 일시정지, 00:50')
  ...
  const time = dialog.locator('[data-pomo-timer-ring] strong')
  await expect(time).toHaveText('00:50')
  await expect(dialog.getByRole('button', {exact: true, name: '계속하기'})).toBeFocused()
  await captureTimer(page, information, 'paused-timer-dark')
  await page.keyboard.press('Escape')
  await page.clock.runFor(200)
  await expect(trigger).toBeFocused()
  await controls.getByRole('button', {exact: true, name: '계속하기'}).click()
  await page.clock.fastForward(50_000)
  await expect(trigger).toHaveAccessibleName('포모도로 열기, 휴식 준비, 01:00')

  await trigger.click()
  const panel = dialog.getByRole('region', {name: '포모도로 타이머'})
  await expect(panel).toHaveAttribute('data-phase', 'shortBreak')
  await expect(dialog.getByLabel('2회 중 1회 집중 완료', {exact: true})).toBeVisible()
  await captureTimer(page, information, 'completed-session-dark')
  await dialog.getByRole('button', {exact: true, name: '휴식 시작'}).click()
  await page.clock.fastForward(10_000)
  await expect(time).toHaveText('00:50')
  await dialog.getByRole('button', {exact: true, name: '세션 초기화'}).click()
  await expect(panel).toHaveAttribute('data-phase', 'focus')
  await expect(time).toHaveText('01:00')
  await expect(dialog.getByLabel('2회 중 0회 집중 완료', {exact: true})).toBeVisible()
  await expect(dialog.getByRole('button', {exact: true, name: '세션 초기화'})).toHaveCount(0)
  await expect(dialog.getByRole('button', {exact: true, name: '현재 세션 종료'})).toHaveCount(0)

  await reloadHome(page)
  await openTimer(page)
  await expect(dialog.getByLabel('2회 중 0회 집중 완료', {exact: true})).toBeVisible()
  await expect(dialog.getByRole('button', {name: /세션 · 집중/u})).toHaveText(
    '2세션 · 집중 1분 · 짧은 휴식 1분 · 긴 휴식 2분',
  )
  await expect(dialog.getByRole('button', {exact: true, name: '집중 시작'})).toBeVisible()
  await page.clock.fastForward(60_000)
  await expect(time).toHaveText('01:00')
  await expect(panel).toHaveAttribute('data-phase', 'focus')
})
