...
이 문서는 완성된 포커스 룸 이미지 한 장을 `base`, `head`, `hands` 등의 레이어로 분리하고 PixiJS에서 다시 조합하는 절차를 정리한다. 다음 작업을 맡은 AI는 장면을 새로 그리기 전에 이 문서를 기준으로 원본 보존 범위와 레이어 경계를 먼저 정한다.
...
- 모든 레이어는 원본과 같은 캔버스 크기와 좌표를 유지한다. 이 프로젝트의 포커스 룸 장면은 `1672×941`이다.
...
6. 실제 `/focus-room`에서 눈, 레이어 애니메이션과 깊이 효과가 함께 움직이는지 확인한다.
