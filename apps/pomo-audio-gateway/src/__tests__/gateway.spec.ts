...
import {type AudioCache, type AudioGatewayConfig, handleAudioGatewayRequest} from '../gateway'

interface TestEnvironment {
  readonly ALLOWED_ORIGINS: string
  readonly ALLOWED_ORIGIN_SUFFIXES: string
  readonly PAID_AUDIO: R2Bucket
...
const audioGateway = {
  fetch(
    ...
    const config: AudioGatewayConfig = {
...
describe('audio gateway request handling', () => {
  it('should return not found for an unknown route', async () => {
    const request = new Request('https://audio.pomofi.io/unknown')
    const environment = {
      ...BASE_ENVIRONMENT,
      PAID_AUDIO: {} as R2Bucket,
      PLAYBACK_TOKEN_SECRET: SECRET,
    ...
    const response = await audioGateway.fetch(request, environment, {} as ExecutionContext)

    expect(response.status).toBe(404)
    expect(await response.json()).toEqual({error: 'route_not_found'})
  })

  it('should reject an untrusted origin before authenticating a media request', async () => {
  ...
  it('should return a cached full response without reading R2', async () => {
    ...
    vi.mocked(cache.match).mockResolvedValue(
  ...
  it('should answer HEAD from cache without reading R2', async () => {
    vi.useFakeTimers()
    vi.setSystemTime('2026-08-22T01:00:00.000Z')
    const token = await createPlaybackToken({...CLAIMS, secret: SECRET})
    const request = new Request(
    ...
    vi.mocked(cache.match).mockResolvedValue(new Response(null, {headers: {ETag: 'cached'}}))
    const head = vi.fn()
    const environment = {
      ...BASE_ENVIRONMENT,
      PAID_AUDIO: {head} as unknown as R2Bucket,
      PLAYBACK_TOKEN_SECRET: SECRET,
    }
  ...
  it('should return not found when HEAD cannot find the authenticated object', async () => {
    vi.useFakeTimers()
    vi.setSystemTime('2026-08-22T01:00:00.000Z')
    const token = await createPlaybackToken({...CLAIMS, secret: SECRET})
    const request = new Request(
    ...
    const environment = {
      ...BASE_ENVIRONMENT,
      PAID_AUDIO: {head: vi.fn().mockResolvedValue(null)} as unknown as R2Bucket,
      PLAYBACK_TOKEN_SECRET: SECRET,
    }
...
describe('audio gateway range requests', () => {
  interface RangeFixture {
    ...
    readonly fullObjectMissing?: boolean
  ...
  const expectRangeResponse = async ({
    ...
    const get = vi
      ...
      .mockResolvedValueOnce(fullObjectMissing ? null : fullObject)
  ...
  it.each(['bytes=abc', 'bytes=0-0,2-2', 'items=0-2'])(
    'should ignore an unsupported Range header and serve the full object for %s',
    async (rangeHeader) => {
      vi.useFakeTimers()
      vi.setSystemTime('2026-08-22T01:00:00.000Z')
      const token = await createPlaybackToken({...CLAIMS, secret: SECRET})
      ...
      const body = new Response(new Uint8Array([0, 1, 2])).body
      ...
      const cacheLookup = vi.mocked(cache.match).mock.calls[0]?.[0]
      expect(cacheLookup?.headers.get('Range')).toBeNull()
      expect(get).toHaveBeenCalledWith(CLAIMS.objectKey)
      expect(response.status).toBe(200)
  ...
  it('should skip cache fill when the full object disappears after a range read', async () => {
    await expectRangeResponse({
      bytes: [4, 5, 6, 7],
      contentRange: 'bytes 4-7/10',
      fullObjectMissing: true,
      objectRange: {length: 4, offset: 4},
      rangeHeader: 'bytes=4-7',
      status: 206,
    })
  ...
  it('should read and cache an object under the configured Preview prefix', async () => {
    ...
    const [cacheRequest, cacheResponse] = vi.mocked(cache.put).mock.calls[0] ?? []
